  ...
  return (
    <div className="flex flex-col min-h-[100dvh] px-5 py-10 max-w-lg mx-auto justify-between">
      <div>
        ...
        <motion.div
          className="mt-6 rounded-2xl glass-panel p-5"
        ...
        >
          <div className="flex items-end justify-between mb-3">
            <div>
              ...
              >
                {avgOvr.toFixed(0)}
                <span className="text-lg text-white/30 ml-1">avg OVR</span>
              </p>
            </div>
          ...
          <div className="h-2 rounded-full bg-white/8 overflow-hidden">
            <motion.div
              ...
              style={{
                background: `linear-gradient(90deg, ${ovrAccentColor(avgOvr)}99, #c9a227)`,
              }}
            />
          </div>
        </motion.div>
        ...
        >
          {picks.map(({ role, player, team }, i) => (
            <motion.div
              key={role}
              className="flex items-center gap-3.5 p-3.5 rounded-2xl glass-panel"
              style={{
                borderColor: `${team.accent}20`,
                background: `linear-gradient(90deg, ${team.accent}0c 0%, transparent 70%)`,
              }}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[9px] text-white/35 uppercase tracking-widest">
                  {ROLE_LABELS[role]} · {team.year}
                </p>
                <p className="font-display text-base text-white truncate">{player.name}</p>
                <p className="text-[10px] text-white/30 truncate">{team.name}</p>
              </div>
